...
class BehandlingDSLTest {
    ...
    fun `dsl test`() {
        ...
        behandling.besvar(grandparent2.uuid, true, testSporing(listOf(Rolle.Saksbehandler)))
        ...
        behandling.besvar(parent1.uuid, 3, testSporing(listOf(Rolle.Saksbehandler)))
        ...
        behandling.besvar(child21.uuid, "Dette er et svare", testSporing(listOf(Rolle.Saksbehandler)))
        behandling.nesteSteg().map { it.id } shouldBe setOf("Parent2", "Child22")
        val child22 = behandling.nesteSteg().single { it.id == "Child22" }
        behandling.besvar(child22.uuid, true, testSporing(listOf(Rolle.Saksbehandler)))
        behandling.nesteSteg().map { it.id } shouldBe setOf("Parent2")
        val parent2 = behandling.nesteSteg().single { it.id == "Parent2" }
        behandling.besvar(parent2.uuid, "Dette er et svar også", testSporing(listOf(Rolle.Saksbehandler)))
        ...
        behandling.besvar(grandparent3.uuid, true, testSporing(listOf(Rolle.Saksbehandler)))
