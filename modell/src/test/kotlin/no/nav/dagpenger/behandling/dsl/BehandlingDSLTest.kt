...
class BehandlingDSLTest {
    ...
    fun `dsl test`() {
        ...
        behandling.alleSteg().map { it.id } shouldBe setOf("GrandparentFelles", "Grandparent2", "Grandparent3", "Parent1", "Parent2", "Child21", "Child22")
        behandling.nesteSteg().map { it.id } shouldBe setOf("GrandparentFelles", "Parent1", "Child21", "Child22")
        behandling.nesteSteg().size shouldBe 4

        // Besvare
        val grandparentFelles = behandling.nesteSteg().single { it.id == "GrandparentFelles" }
        behandling.besvar(grandparentFelles.uuid, LocalDate.now())
        behandling.nesteSteg().map { it.id } shouldBe setOf("Grandparent2", "Parent1", "Child21", "Child22")

        val grandparent2 = behandling.nesteSteg().single { it.id == "Grandparent2" }
        behandling.besvar(grandparent2.uuid, true)
        behandling.nesteSteg().map { it.id } shouldBe setOf("Parent1", "Child21", "Child22")

        val parent1 = behandling.nesteSteg().single { it.id == "Parent1" }
        behandling.besvar(parent1.uuid, 3)
        behandling.nesteSteg().map { it.id } shouldBe setOf("Child21", "Child22")

        val child21 = behandling.nesteSteg().single { it.id == "Child21" }
        behandling.besvar(child21.uuid, "Dette er et svare")
        behandling.nesteSteg().map { it.id } shouldBe setOf("Child22")

        val child22 = behandling.nesteSteg().single { it.id == "Child22" }
        behandling.besvar(child22.uuid, true)
        behandling.nesteSteg().map { it.id } shouldBe setOf("Parent2")

        val parent2 = behandling.nesteSteg().single { it.id == "Parent2" }
        behandling.besvar(parent2.uuid, "Dette er et svar også")
        ...
        behandling.besvar(grandparent3.uuid, true)
