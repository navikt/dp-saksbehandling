...
class RapidFilterVerifiserOpplysningMottakTest {
    private val testRapid = TestRapid()
    private val testMessage =
        mapOf(
            "@event_name" to "verifiser_opplysning",
            "@id" to "id1",
    ...
    fun `Skal ikke behandle pakker som mangler required keys`(
        ...
        val testListener = TestListener(testRapid)
        val mutertTestMessage =
            JsonMessage.newMessage(testMessage.muterOgKonverterToJsonString { it.remove(requiredKey) }).toJson()
    ...
    private class TestListener(rapidsConnection: RapidsConnection) : River.PacketListener {
        ...
        init {
            River(rapidsConnection).apply(
                VerifiserOpplysningMottak.rapidFilter,
            ).register(this)
        }
